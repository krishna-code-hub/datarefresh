...
async def apply_masking(data, columns, metadata):
    ...
    for row in data:
        ...
        row_dict = {column["column_name"]: value for column, value in zip(columns, row)}

        for column in columns:
            value = row_dict[column["column_name"]]
            if column.get("pii") == "Y":
                if column.get("masking_algorithm"):
                    ...
                    logging.debug(f' masking for column {column["column_name"]}')
                    logging.debug(f" masking for format_type {format_type}")
                    masked_value = await fpe_encrypt_async(
                        value, mode, format_type, tweak_length, key_env_var
                    )
                else:
...
async def fpe_encrypt_async(value, mode, format_type, tweak_length, key_env_var):
    ...
    # TODO - to be replaced with vault
    key = get_key()
